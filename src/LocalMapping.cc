...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            {
                if(mpAtlas->KeyFramesInMap(this->sysId)>2)
                {
                ...
                if ((mTinit<100.0f) && mbInertial)
                {
                    if(mpCurrentKeyFrame->GetMap()->isImuInitialized() && mpTracker->mState==Tracking::OK)
                    {
                        ...
                        // scale refinement
                        if (((mpAtlas->KeyFramesInMap(this->sysId))<=100) &&
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            cv::Mat x3D_(x3D);
            MapPoint* pMP = new MapPoint(x3D_,mpCurrentKeyFrame,mpAtlas->GetCurrentMap(this->sysId));
...
void LocalMapping::KeyFrameCulling()
{
    ...
    const bool bInitImu = mpAtlas->isImuInitialized(this->sysId);
    int count=0;
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        if(nRedundantObservations>redundant_th*nMPs)
        {
            if (mbInertial)
            {
                if (mpAtlas->KeyFramesInMap(this->sysId)<=Nd)
                    continue;
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if(mpAtlas->KeyFramesInMap(this->sysId)<nMinKF)
        return;
    ...
    Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(this->sysId), mRwg, mScale, mbg, mba, mbMonocular, infoInertial, false, false, priorG, priorA);
    ...
    {
        mpAtlas->GetCurrentMap(this->sysId)->ApplyScaledRotation(Converter::toCvMat(mRwg).t(),mScale,true);
    ...
    {
        if (priorA!=0.f)
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(this->sysId), 100, false, 0, NULL, true, priorG, priorA);
        else
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(this->sysId), 100, false, 0, NULL, false);
    }
    ...
    if (!mpAtlas->isImuInitialized(this->sysId))
    {
        cout << "IMU in Map " << mpAtlas->GetCurrentMap(this->sysId)->GetId() << " is initialized" << endl;
        mpAtlas->SetImuInitialized(this->sysId);
...
void LocalMapping::ScaleRefinement()
{
    ...
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(this->sysId), mRwg, mScale);
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
