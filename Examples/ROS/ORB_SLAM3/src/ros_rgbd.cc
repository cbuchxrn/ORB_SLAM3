...
void showBlended(const cv_bridge::CvImageConstPtr cv_ptrRGB, const cv_bridge::CvImageConstPtr cv_ptrD)
{
    cv::Mat rgbImg      = cv_ptrRGB->image.clone();
    cv::Mat blendedImg  = cv::Mat(rgbImg.size(),rgbImg.type());
...
{   
    ...
    // show both imgs overlayed
    showBlended(cv_ptrRGB,cv_ptrD);
